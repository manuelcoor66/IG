...
// tipos
typedef enum{CUBO, PIRAMIDE, OBJETO_PLY, ROTACION, CILINDRO, CONO, ESFERA, EXTRUSION, COCHE} _tipo_objeto;
...
int Window_x=50,Window_y=50,Window_width=650,Window_high=650, activo=-1;
...
// Luces
bool luz2_on = false, ortogonal = false;
float alfa = 0.0;
...
switch (t_objeto){
	case CUBO: cubo.draw(modo,1.0,0.0,0.0,5,oro);break;
	case PIRAMIDE: piramide.draw(modo,1.0,0.0,0.0,5,oro);break;
        case OBJETO_PLY: ply.draw(modo,1.0,0.6,0.0,5,oro);break;
        case ROTACION: rotacion.draw(modo,1.0,0.0,0.0,5,oro);break;
        case CILINDRO: cilindro.draw(modo,1.0,0.0,0.0,5,oro);break;
        case CONO: cono.draw(modo,1.0,0.0,0.0,5,oro);break;
        case ESFERA: esfera.draw(modo,1.0,0.0,0.0,5,oro);break;
        case COCHE: coche.draw(modo,1.0,0.8,0.2,5,oro);break;
        case EXTRUSION: extrusion->draw(modo,1.0,0.0,0.0,5,oro);break;
	}
...
for (i=0;i<coche.piezas;i++)
   {if (color[0]==coche.color_select[i].r &&
        color[1]==coche.color_select[i].g &&
        color[2]==coche.color_select[i].r)
       {if (coche.activo[i]==0) 
                      {coche.activo[i]=1;
                      }
                  else 
...
printf("Activo: %d, valor x %d, valor y %d, color %d, %d, %d \n",activo,x,y,pixel[0],pixel[1],pixel[2]);
...
cout << "Tecla T: Vista normal" << endl;
cout << "Tecla Y: Vista ortogonal" << endl;
...
cout << "Número 5: Sólido plano (no soportado en la coche)" << endl;
cout << "Número 6: Sólido suave (no soportado en la coche)" << endl;
